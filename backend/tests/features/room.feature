Feature: Creating rooms
  ...
  Scenario: Room could not be created because price is not a numerical value
    Given o usuário tem um id de pj "f5b0e3d2-4b6f-4d8f-8f5a-7b1a5b2f8a1d"
    When uma requisição POST for enviada para "/api/rooms" com o corpo da requisição sendo um JSON com:
      | campo              | var         |
      | description        | "Room Seed" |
      ...
      | avaliacao          | 5           |
    Then o status da resposta deve ser "400"
    And o JSON da resposta deve conter a mensagem de erro "O preço deve ser um valor numérico"

  Scenario: Room could not be created because stars is not a integer between 1 and 5
    Given o usuário tem um id de pj "f5b0e3d2-4b6f-4d8f-8f5a-7b1a5b2f8a1d"
    When uma requisição POST for enviada para "/api/rooms" com o corpo da requisição sendo um JSON com:
      | campo              | var         |
      | description        | "Room Seed" |
      ...
      | avaliacao          | 5           |
    Then o status da resposta deve ser "400"
    And o JSON da resposta deve conter a mensagem de erro "O número de estrelas do quarto deve ser um número inteiro entre 1 e 5"