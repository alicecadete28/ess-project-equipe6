...
const feature = loadFeature('tests/features/findReservation-controller.feature');
...
defineFeature(feature, (test) => {
    ...
    test('Encontrar quartos disponíveis', ({ given, and, when, then }) => {
        given(/^o RoomRepository retorna uma lista de acomodações disponíveis para "(.*)"$/,
            async (destino) => {
        ...
        and('o JSON da resposta deve ser uma lista de acomodações disponíveis', () => {
            ...
            const rooms = response.body.every((room: RoomEntity) => room.local == 'Recife');
    ...
    test('Buscar acomodações para 4 hóspedes', ({ given, and, when, then }) => {
        given(/^o RoomRepository retorna uma lista de acomodações adequadas para "(.*)" hóspedes em "(.*)"$/, 
        ...
        and(/^o JSON da resposta deve ser uma lista de acomodações para (\d+) hóspedes$/, (numHospedes) => {
            expect(response.body).toBeInstanceOf(Array);
            const fourPeopleRooms = response.body.every((room: RoomEntity) => room.capacity >= Number(numHospedes));
    ...
    test('Buscar acomodações para 10 hóspedes sem acomodações disponíveis', ({ given, and, when, then }) => {
        given(/^o RoomRepository não tem acomodação para "(.*)" hóspedes em "(.*)"$/, async (numHospedes, destino) => {
    ...
    test('Nenhuma acomodação disponível para o destino e datas selecionadas', ({ given, when, then, and }) => {
        given(/^o RoomRepository não tem acomodações disponíveis para "(.*)" entre "(.*)" e "(.*)"$/, async (destino, dataIda, dataVolta) => {
            const rooms = await mockTestRepository.getRooms();
        ...
        and(/^o JSON da resposta deve conter a mensagem "(.*)" deve ser retornada$/, (expectedMessage) => {
            expect(response.body.message).toBe(expectedMessage);
        });
    });
});
