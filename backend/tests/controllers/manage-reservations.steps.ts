...
defineFeature(feature, (test) => {
    ...
    test('Return reservations by pf_id', ({ given, when, then, and }) => {
        given(/^o ReservationRepository tem uma reserva com id "(.*)", pf_id "(.*)", room_id "(.*)", check_in "(.*)", check_out "(.*)", guests "(.*)", total "(.*)", status "(.*)", rating "(.*)"$/,
            async (id, pf_id, room_id, check_in, check_ou, guest, total, status, rating) => {
              const user = await mockReservationRepository.getReservationByPFId(id);
        ...
        and(/^a resposta deve ser um JSON com a seguinte reserva: id "(.*)", pf_id "(.*)", room_id "(.*)", check_in "(.*)", check_out "(.*)", guests "(.*)", total "(.*)", status "(.*)", rating_comment "(.*)", rating_star "(.*)"$/,
            async (id, pf_id, room_id, check_in, check_out, guests, total, status, rating_c, rating_s) => {
                expect(response.body).toEqual([{
                    id: id,
                    check_in: check_in,
                    check_out: check_out,
