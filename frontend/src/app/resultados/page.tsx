...
export default function SearchResults() {
  ...
  const sortRooms = (options = sortOptions) => {
    ...
    if (options.price) sortedRooms.sort((a, b) => a.price - b.price);
    else if (options.stars) sortedRooms.sort((a, b) => b.stars - a.stars);
    else if (options.rating) sortedRooms.sort((a, b) => b.rating - a.rating);
  ...
  const fetchFilteredRooms = async () => {
    ...
    if (filters.airConditioning) query.append("ar_condicionado", "true");
    if (filters.tv) query.append("tv", "true");
    if (filters.wifi) query.append("wifi", "true");
    if (filters.petFriendly) query.append("petFriendly", "true");
    if (filters.breakfast) query.append("cafeDaManha", "true");
    if (filters.parking) query.append("estacionamento", "true");
  ...
  useEffect(() => { fetchFilteredRooms(); }, []);
  useEffect(() => { sortRooms(); }, [sortOptions]);

  const handleSearch = () => fetchFilteredRooms();
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex flex-1">
        <div className="flex-1 p-4">
          {errorMsg && <div className="text-center text-red-600 font-semibold my-4">{errorMsg}</div>}
            ...
            <div key={room.id} className="flex mb-4 border rounded-md overflow-hidden shadow-md">
              ...
              <div className="flex-1 p-4">
                ...
                <p className="text-gray-600 mt-2">
                  {Array.isArray(room.caracteristics_ids)
                    ? room.caracteristics_ids.join(", ")
                    : String(room.caracteristics_ids)}
                </p>
                ...
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {room.wifi && <Badge className="flex items-center gap-1"><Wifi className="h-3 w-3 mr-1" /> Wi-Fi</Badge>}
                    {room.petFriendly && <Badge className="flex items-center gap-1"><Dog className="h-3 w-3 mr-1" /> Pet-Friendly</Badge>}
                    {room.cafeDaManha && <Badge className="flex items-center gap-1"><Coffee className="h-3 w-3 mr-1" /> Café da Manhã</Badge>}
                    {room.estacionamento && <Badge className="flex items-center gap-1"><Car className="h-3 w-3 mr-1" /> Estacionamento</Badge>}
                  </div>

                  {/* Avaliação sempre abaixo */}
                  <div className="mt-2">
                ...
                <div className="flex justify-between items-end mt-6">
                  ...
                  <Button className="bg-[#0079c2] hover:bg-[#0069b0] text-white" onClick={() => handleReserve(room)}>
                    Reservar
                  </Button>
                </div>
              </div>
        ...
        <div className="w-64 bg-gray-100 p-4">
          ...
          {["Ar condicionado", "Wi-Fi", "Estacionamento", "Café da Manhã", "Pet-Friendly", "TV"].map((label, idx) => {
            const keys = ["airConditioning", "wifi", "parking", "breakfast", "petFriendly", "tv"];
            const key = keys[idx] as keyof typeof filters;
            return (
          ...
          {["Preço", "Estrelas", "Avaliação"].map((label, idx) => {
            const keys = ["price", "stars", "rating"];
            const key = keys[idx] as keyof typeof sortOptions;
            return (
              <div key={key} className="flex items-center space-x-2">
                <Checkbox id={key} checked={sortOptions[key]} onCheckedChange={() => handleSortChange(key)} />
                <label htmlFor={key} className="text-sm font-medium">{label}</label>
              </div>
            );
          })}
