...
export default function Avaliations() {
  ...
  const handleSubmit = async () => {
    ...
    if (!reservationId) {
      setErrorMessage("ID da reserva não encontrado. Por favor, tente novamente.")
      setShowError(true)
      return
    }
    ...
    try {
      ...
      console.log("Submitting review for reservation ID:", reservationId)

      // Submit to backend using POST request with the ID from URL
      const response = await fetch(`http://localhost:5001/api/avaliacoes?id=${reservationId}`, {
        method: "POST",
        headers: {
